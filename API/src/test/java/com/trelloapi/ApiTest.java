...
//	{
		...
		String resp=RestAssured.given()
			...
			.queryParam("idList", idOfToDo).header("Content-Type","application/json")
		.when().post("/1/cards")
		.then().log().all().statusCode(200).body("name", equalTo(cardName)).extract().response().asString();
...
//	{
		...
		.queryParam("name", updateCardName).header("Content-Type","application/json")
		.when().post("/1/cards/")
...
//	{
		...
		RestAssured.given()
		.queryParam("key", "d726239b5f1510d74a1a32bc575f01a2")
		.queryParam("token", "5d6e8b5e8704537e61b5bad9ec99e76e30d4503b0e657f16845cb87a3c88f8b6").header("Content-Type","application/json")
