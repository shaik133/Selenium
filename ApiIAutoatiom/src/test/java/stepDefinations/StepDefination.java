...
import resourses.PojoClass_Parent;
...
//	    request=RestAssured.given().spec(req).body(new String(Files.readAllBytes(Paths.get("C:\\Users\\Admin\\Documents\\addPlace.json"))));
	    ...
	    request=RestAssured.given().spec(req).body(PojoClass_Parent.getpayload());
	}
	...
	@Then("user validate success response with {string} {string}")
	public void user_validate_success_response_with(String name, String value) {
	    // Write code here that turns the phrase above into concrete actions
	  String resp= response.asString();
	  System.out.println(resp);
	  JsonPath js = new JsonPath(resp);
	  assertEquals(js.getString(name),value);
	}
